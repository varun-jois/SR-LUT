...
id = 'id04119'  # S, colors, id04119, x4_4bit_int8
testid = 'id04119'
...
files_lr = glob.glob(TEST_DIR + '/val_LR/*.png'.format(UPSCALE))
files_lr.sort()

# Test GT images
files_gt = glob.glob(TEST_DIR + '/val_HR/*.png')
files_gt.sort()
...
for lut_type in ['overfit']:  # 'pretrained',
    ...
    print(f'AVG PSNR {lut_type}: {np.mean(np.asarray(psnrs))}')
